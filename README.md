...
## 📖 Giới thiệu dự án
**Sportify** là nền tảng quản lý và đặt lịch thuê sân thể thao trực tuyến (Bóng đá, Tennis, Cầu lông...) được xây dựng trên nền tảng Java Spring Boot. Website có tích hợp  **Trợ lý ảo AI (SportAI)** thông qua mô hình ngôn ngữ lớn (LLM). Bên cạnh đó, dự án cung cấp quy trình thanh toán khép kín với **Cổng thanh toán điện tử VNPAY**, hỗ trợ đặt cọc  để giữ chỗ hoàn toàn tự động.
...
# Nhập Email  và Mật khẩu ứng dụng (App Password) của Gmail
...
```
*(Hiện tại code đã cấu hình sẵn mã Test Sandbox. Nếu  đưa lên thực tế, hãy thay bằng mã do VNPAY cung cấp cho doanh nghiệp của bạn).*
...
- **Admin:** (Email: `adminsportify` / Pass: `adminsportify`)
- **Nhân viên (Staff):** (Email: `nhanvien` / Pass: `nhanvien`)
